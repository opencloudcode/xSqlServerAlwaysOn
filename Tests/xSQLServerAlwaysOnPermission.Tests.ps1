$moduleName = 'xSQLServerAlwaysOnPermission'
...
InModuleScope $moduleName {
    Describe "$moduleName\Get-TargetResource" {
        Context 'Testing SQLPS module existance' {
            ...
            It 'get throws an exception if module SQLPS does not exist' {
                { Get-TargetResource -NodeName 'localhost' -InstanceName 'MSSQLSERVER' -Principal "Dummy" -Permission 'VIEW SERVER STATE' } | Should Throw
            }

            It 'set throws an exception if module SQLPS does not exist' {
                { Set-TargetResource -NodeName 'localhost' -InstanceName 'MSSQLSERVER' -Principal "Dummy" -Permission 'VIEW SERVER STATE' } | Should Throw
            }

            It 'test throws an exception if module SQLPS does not exist' {
                { Test-TargetResource -NodeName 'localhost' -InstanceName 'MSSQLSERVER' -Principal "Dummy" -Permission 'VIEW SERVER STATE' } | Should Throw
            }

            Assert-MockCalled Import-SQLPSModule -ModuleName $moduleHelperName -Scope Context -Exactly 3
            Assert-VerifiableMocks
        }

        Context 'Testing SQL instance existance' {
            Mock Get-SQLPSInstance -Verifiable -MockWith { 
                throw 'SQL Service instance not found'
            }
            
            It 'throws an exception if sql server is inaccessible' {
                { Get-TargetResource -NodeName 'localhost' -InstanceName 'MSSQLSERVER' -Principal "Dummy" -Permission 'VIEW SERVER STATE' } | Should Throw
            }
            ...
            Assert-MockCalled Get-SQLPSInstance -Exactly 3
            Assert-VerifiableMocks 
        } 
    }
...
<#
Describe $moduleName {
    Context 'Testing if functions throws if SQLPS module does not exist' {
        Mock Import-SQLPSModule -ModuleName $moduleHelperName -Verifiable -MockWith {
            throw 'Module SQLPS not found'
        }
